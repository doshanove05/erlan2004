...
vk.setOptions({
    ...
    apiMode: 'parallel',
    pollingGroupId: 1 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if(!uid[message.user]){
		...
		message.send(`🎉 » *id${message.user}, вы успешно зарегистрировались!\n📝 » "Помощь" - список команд.\n📝 » Чтобы не пропустить обновление проекта вы должны быть подписаны на нашу группы: "[s.botik|SBot | Игровой бот]"`)
   ...
   🐼 » Питомцы
    ...
    📋 » Дом - Информация о всём имуществе.
 			`);
 	});
...
📒 » Профиль - Узнать о себе.
📙 » Профиль (ID) - Просмотр профиля игрока.
📃 » Поиск (Сыллка) - Должность,Прозвище, ИД.
💲 » Баланс - Ваши деньги.
💳 » Банк - Взять кредит.
✄ » Ник (Желаемый ник).
...
⚄  » В группе [club155070061|SBot - Игровой бот] бот работает без задержки.
	...
	for(z in houses){
 	...
 	if(user.house == false) return message.send(`👉 » Для покупки машины Вам нужен дом!`);  
		if(!message.$match[1]){
			...
			　6. BMW X6 - 120.000.000$
			　6. Porsche Cayenne - 155.000.000$
			　6. BMW 7 Series - 164.000.000$
			　6. Lexus LX - 190.000.000$
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:тикет|тик|tiket|tik|репорт|report|реп|rep|жалоба|вопрос)\s?([^]+)?/i, (message) => { 
		let user = acc.users[user_id(message.user)];
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		let dev = '';   
		return message.send(`
			📝 » Проект: [s.botik|SBot - Игровой бот] (v2.7)
			💼 » Создатель: [kotik_2018|Макс Екимцов] 


			📗 » Пользователей: ${acc.number} (Цель: 500).
			`);
	});
	vk.updates.hear(/^(?:беседы)$/i,  (message) => { 
		return message.send(`
			📘 » Ссылки на наши беседы: 
   1. vk.me/join/AJQ1dxndKgpT_h4/ve25iv4g
   2. Здесь модет появится ваша беседа. (Подробнее в группе).
...
return message.send(` *id${message.user} (${acc.users[user_id].prefix}) (${id}) : Сообщение успешно доставлен *id${message.user}${acc.users[message.$match[1]].prefix}`);
    ...
    vk.updates.hear(/^(?:чмоки)\s?([^]+)?/i,  (message) => {
        let id = acc.users[message.$match[1]]
        let user = acc.users[user_id(message.user)];
        vk.api.call('messages.send',  { 
        peer_id: acc.users[message.$match[1]].id,
        message: `Каро долбоёб который через node запускает на хостинге БОТА 💋`
        });
        return message.send(`Ну блять и кодер конечно это сделал...`);
    });
